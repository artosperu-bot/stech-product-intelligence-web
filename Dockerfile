...
# The transport bundle may be missing only the gzip/tar trailer. Extraction is allowed
# to continue only if all 26 V30 Python files match the exact expected digest afterwards.
RUN cat /tmp/legacy_core_bundle/part-0?.b64 /tmp/legacy_core_bundle/part-10.b64 /tmp/legacy_core_bundle/part-11.b64 | base64 -d > /tmp/legacy_core.tar.gz && \
    (tar -xzf /tmp/legacy_core.tar.gz -C /app || true) && \
